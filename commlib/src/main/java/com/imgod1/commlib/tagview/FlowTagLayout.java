...
//              new MarginLayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT));//这个构造方法所然能使用但是编译器会报错
     ...
     * 设置某项不可点击或选择
     */
    public void setPositionEnable(int...position){
        int count = mAdapter.getCount();
        for (int pos : position) {
            if (pos > count - 1) {
                throw new ArrayIndexOutOfBoundsException("position must not more than source count");
            }
            ViewGroup linearLayout= (LinearLayout) getChildAt(pos);
